...
const CreateFiles = (_from, _to, _filter) => __awaiter(void 0, void 0, void 0, function* () {
    const to = _to;
    const from = _from;
    ...
    yield exec(`git archive ${to} --format=tar \`git diff --name-only ${from} ${to} --diff-filter=${_filter || "ACMR"}\` -o ${outPutFile}`);
